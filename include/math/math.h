...
namespace Rainy
{
    ...
    {
    public:
        Vector2(T p_value = 0) : x(p_value), y(p_value) {}
        ...
        T dot(Vector2<T> const &v) const { return x * v.x + y * v.y; }
        ...
        Vector2<T> &operator*=(T const &scalar) { return this->Mul(scalar); }

        Vector2<T> &operator+=(Vector2<T> const &v) { return this->add(v); }
    ...
    {
    public:
        Vector3(T p_value = 0) : x(p_value), y(p_value), z(p_value) {}

        Vector3(T p_x, T p_y, T p_z) : x(p_x), y(p_y), z(p_z) {}

        Vector3(Vector2<T> const &v) : x(v.x), y(v.y), z(0) {}
        ...
        float lengthQuad() const { return x * x + y * y + z * z; }

        T dot(Vector3<T> const &v) const { return x * v.x + y * v.y + z * v.z; }
        ...
        bool operator==(Vector3<T> const &v) const { return (x == v.x && y == v.y && z == v.z); }
    ...
    {
    public:
        Vector4(T p_value = 0) : x(p_value), y(p_value), z(p_value), w(p_value) {}

        Vector4(T p_x, T p_y, T p_z, T p_w) : x(p_x), y(p_y), z(p_z), w(p_w) {}

        Vector4(Vector2<T> const &v, T const &z = 0, T const &w = 0) : x(v.x), y(v.y), z(0), w(w) {}

        Vector4(Vector3<T> const &v, T const &w = 0) : x(v.x), y(v.y), z(v.z), w(w) {}

        Vector4(Vector4<T> const &v) : x(v.x), y(v.y), z(v.z), w(v.w) {}
    ...
    {
    public:
        Matrix2(Vector2<T> const &v0 = { 1, 0, 0 }, Vector2<T> const &v1 = { 0, 1, 0 })
        ...
        Matrix2(Matrix2<T> const &m) : m_data(new Vector2<T>[2])
        {
            m_data[0] = m[0];
            m_data[1] = m[1];
        }
    ...
    {
    public:
        Matrix3(Vector3<T> const &v0 = { 1, 0, 0 },
                Vector3<T> const &v1 = { 0, 1, 0 },
        ...
        Matrix3(Matrix3<T> const &m) : m_data(new Vector3<T>[3])
        {
            m_data[0] = m[0];
            m_data[1] = m[1];
            m_data[2] = m[2];
    ...
    {
    public:
        Matrix4(Vector4<T> const &v0 = { 1, 0, 0, 0 },
                Vector4<T> const &v1 = { 0, 1, 0, 0 },
                Vector4<T> const &v2 = { 0, 0, 1, 0 },
                Vector4<T> const &v3 = { 0, 0, 0, 1 })
        ...
        Matrix4(Matrix4<T> const &m) : m_data(new Vector4<T>[4])
        {
            m_data[0] = m[0];
            m_data[1] = m[1];
            m_data[2] = m[2];
        ...
        Matrix4<T> &Rotate(float const &angle, Vector3<T> const &axis)
        {
            ...
            Matrix4<T> m_m = { { cos(rad_angle) + (1 - cos(rad_angle)) * axis.x * axis.x,
    ...
    {
        return { v0.x * scalar, v0.y * scalar, v0.z * scalar, v0.y * scalar };
    }
