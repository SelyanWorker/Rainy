...
namespace Rainy {
	...
	Matrix4f CreateOrthographicMatrix(float left, float right, float bottom, float top,
	...
	{
		Vector4f clipSpaceV = {
			(viewport_position.x / viewport_width) * 2 - 1,
			-((viewport_position.y / viewport_height) * 2 - 1), 1, 1 };
...
*/
	...
	bool AABB2DOverlay(AABB2Di first, AABB2Di second, AABB2Di& dest)
	{
		...
		if (!AABB2DInter(first, second)) return false;
		...
		Rainy::AABB2Di overlay = {
			{ int32_t(fmaxf(first.Min.x, second.Min.x)), int32_t(fmaxf(first.Min.y, second.Min.y)) },
			{ int32_t(fminf(first.Max.x, second.Max.x)), int32_t(fminf(first.Max.y, second.Max.y)) }
		};
	...
	std::pair<AABB2Df, AABB2Df> AABB2DSplit(AABB2Df aabb, float splitVal, bool hSplit)
	{
		...
		if (hSplit)
		{
			if (splitVal < aabb.Min.y && splitVal > aabb.Max.y) return result;
		...
		{
			if (splitVal < aabb.Min.x && splitVal > aabb.Max.x) return result;
	...
	std::vector<AABB2Di> AABB2DExcludeOverlay(AABB2Di aabb, AABB2Di overlay)
	{
		...
		if (!AABB2DZeroSpace(split.first)) parts.push_back(split.first);

		split = AABB2DSplit(split.second, overlay.Max.y, true);
		if (!AABB2DZeroSpace(split.second)) parts.push_back(split.second);
