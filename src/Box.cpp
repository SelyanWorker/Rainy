...
namespace Rainy {
	...
	const float lt = (Vector3f{ 0, 0, -1 } + Vector3f{ 0, -1, 0 } + Vector3f{ 1, 0, 0 }).normalize().x;

	Box::Vertex Box::m_vertices[8] =
	{
	...
	Box::Box(Vector3f size)
	{
		...
		auto elements = { BufferElement(FLOAT3, false), BufferElement(FLOAT2, false), BufferElement(FLOAT3, false) };
	...
	void Box::Draw()
	{
		DrawVertexArray(RenderMode::RN_TRIANGLES, m_vertexArray);
	}
