...
public:
    ...
    void onImGuiRender()
    {
        ...
        if (b_ShowGeneratorMenu)
            ShowGeneratorMenu();
        if (b_ShowTerrainCreationMenu)
            ShowTerrainCreationMenu();
        if (b_ShowUtilsMenu)
            ShowUtilsMenu();
...
private:
    void ShowToolAndBrushMenu()
    {
        ImGui::Begin("Tool and Brush", &b_ShowToolAndBrushMenu, ImGuiWindowFlags_AlwaysAutoResize);
        {
            ...
            char const *toolNames[] = { "Sculpt", "Smooth", "Add section", "Delete section" };
        ...
        if (toolType == te::ToolTypes::SCULPT || toolType == te::ToolTypes::SMOOTH)
        {
            ...
            {
                ImGui::SliderFloat("Size", &brush->size, MIN_BRUSH_SIZE, MAX_BRUSH_SIZE);
                ImGui::SliderFloat("Falloff",
                                   &brush->falloff,
                                   MIN_BRUSH_FALLOFF,
                                   MAX_BRUSH_FALLOFF);

                char const *falloffFunctions[] = { "No falloff", "Linear", "Smooth" };
                static int currentFalloffFun = 0;

                ImGui::Combo("Falloff type",
                             &currentFalloffFun,
                             falloffFunctions,
                             IM_ARRAYSIZE(falloffFunctions));
                {
    ...
    void ShowGeneratorMenu()
    {
        ...
        auto clear_fun = [&]()
        {
            for (uint16_t i = 0; i < 128 && imageDir[i]; ++i)
                imageDir[i] = 0;
            for (uint16_t i = 0; i < 32 && imageName[i]; ++i)
                imageName[i] = 0;
        };
        ...
        if (ImGui::Button("Generate"))
        {
            ...
            Rainy::ImageTypes imageType =
                imageTypeNumber == 0 ? Rainy::ImageTypes::RN_PNG : Rainy::ImageTypes::RN_JPEG;
            if (imageType == Rainy::ImageTypes::RN_PNG)
                path += ".png";
            else
                path += ".jpeg";
    ...
    void ShowTerrainCreationMenu()
    {
        ...
        heightMapPath.resize(128);
        static std::string normalMapPath;
        normalMapPath.resize(128);
