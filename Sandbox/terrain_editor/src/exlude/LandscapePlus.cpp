...
namespace ls {
	...
	{
		RN_ASSERT(vertexStep > 0, "Error in section constructor vertexStep: {0}", vertexStep);

		uint32_t verticesCount = stepCount + 1;
		m_vertices.reserve(verticesCount * verticesCount);
		for (uint32_t z = 0; z < verticesCount; z++)
		{
			for (uint32_t x = 0; x < verticesCount; x++)
			{
				m_vertices.push_back({
					Vector3f{x * vertexStep, 0, z * vertexStep} +position,
					{0, 0}, {0, 1, 0}
					});
			}
		}

		for (uint32_t z = 0; z < stepCount; z++)
		{
			for (uint32_t x = 0; x <= stepCount; x++)
			{
				m_indices.push_back(z * verticesCount + x);
				m_indices.push_back((z + 1) * verticesCount + x);
			}
		...
		m_sectionWorldSize = vertexStep * stepCount;
		m_sectionParts = stepCount;
	}
	...
	SectionPart* SectionPlus::GetPart(Vector3f center, float radius)
	{
		...
		float sectionPartSize = m_sectionWorldSize / m_sectionParts;
		std::pair<uint32_t, uint32_t> begin
		{
			uint32_t(std::ceilf(overlay.LeftBorder / sectionPartSize)),
			uint32_t(std::ceilf(overlay.BottomBorder / sectionPartSize))
		};
		std::pair<uint32_t, uint32_t> end
		{
			uint32_t(std::floorf(overlay.RightBorder / sectionPartSize)),
			uint32_t(std::floorf(overlay.TopBorder / sectionPartSize))
		};

		if (end.first > m_sectionParts) end.first = m_sectionParts;
		if (end.second > m_sectionParts) end.second = m_sectionParts;
		...
		for (uint32_t z = begin.second; z <= end.second; z++)
		{
			...
			{
				part->Vertices.push_back(m_vertices[z * (m_sectionParts + 1) + x]);
			}
		}
		return part;
	}
	...
	void SectionPlus::ReloadPart(SectionPart* part)
	{
		...
		for (uint32_t z = begin.second; z <= end.second; z++)
		{
			for (uint32_t x = begin.first; x <= end.first; x++)
			{
				m_vertices[z * (m_sectionParts + 1) + x] =
					part->Vertices[(z - begin.second) * partWidth + x - begin.first];
			}
		}
		uint32_t bytesInWidth = partWidth * sizeof(VertexPlus);
		for (uint32_t z = begin.second; z <= end.second; z++)
		{
			uint32_t firstVertex = (z * (m_sectionParts + 1) + begin.first);
	...
	bool SectionPlus::GetPointHeight(float x, float z, float& height)
	{
		...
		uint32_t vertexXPos = xOffset / vertexStep;
		uint32_t vertexZPos = zOffset / vertexStep;

		uint32_t verticesCount = m_sectionParts + 1;
		...
		Vector3f vPos0 = m_vertices[vertexZPos * verticesCount + vertexXPos].Position;
		Vector3f vPos1 = m_vertices[(vertexZPos + 1) * verticesCount + vertexXPos].Position;
		Vector3f vPos2 = m_vertices[vertexZPos * verticesCount + vertexXPos + 1].Position;

		if ((m_sectionWorldSize - (vertexZPos * vertexStep - leftButtomVertexPosition.z)) >
			(vertexXPos * vertexStep - leftButtomVertexPosition.x))
			vPos0 = m_vertices[(vertexZPos + 1) * verticesCount + vertexXPos + 1].Position;
	...
	LandscapePlus::LandscapePlus(uint32_t width, uint32_t length, float sectionWorldSize, uint32_t sectionPartsCount)
	:	m_mapWidth(width * 2),
		m_mapLength(length * 2),
		m_sectionWorldSize(sectionWorldSize),
		m_vertexStep(sectionWorldSize / sectionPartsCount),
		m_sectionPartsCount(sectionPartsCount)
	{
		...
		for (uint32_t z = 0; z < length; z++)
		{
			for (uint32_t x = 0; x < width; x++)
			{
				...
				SectionPlus* section = new SectionPlus(sectionStartPosition, m_vertexStep, sectionPartsCount);
	...
	{
		float topRightXOffset = m_position.x + (m_mapWidth / 2 * m_sectionWorldSize) - x;
		float topRightZOffset = m_position.z + (m_mapLength / 2 * m_sectionWorldSize) - z;

		float leftBottomXOffset = m_position.x - (m_mapWidth / 2 * m_sectionWorldSize) - x;
		float leftBottomZOffset = m_position.z - (m_mapLength / 2 * m_sectionWorldSize) - z;
		...
		if (StartAboveEndBelow(startPoint, endPoint))
		{
			for (uint32_t i = 0; i < RAY_PARTS_COUNT; ++i)
			{
				...
				if (StartAboveEndBelow(start_ray_part_point, end_ray_part_point))
				{
					...
					return GetSection(start_ray_part_point) != nullptr && GetSection(end_ray_part_point) != nullptr;
				}
			}
		}
