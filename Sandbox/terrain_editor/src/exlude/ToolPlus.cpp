...
namespace ls {
	...
	void BasicTool::ModifyVertexNormals(SectionPart* part)
	{
		...
		for (size_t i = 0; i < part->Length - 1; ++i) {
			for (size_t j = 0; j < part->Width - 1; ++j) {
				...
				if (((i * part->Width + j) / (m_landscape->getWidth() + 1)) % 2 == 0) {
	...
	{
		auto sections = m_landscape->GetSections(m_lastInterPoint, brush->size);
		for (SectionPlus* section : sections)
		{
			SectionPart* part = section->GetPart(m_lastInterPoint, brush->size);
