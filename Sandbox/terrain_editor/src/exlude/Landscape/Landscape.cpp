...
Landscape::Landscape(
	...
	:position(position), section_size(section_size), section_world_size(section_world_size),
...
bool Landscape::getSurfacePoint(float x, float z, Vector3f& dest) const
{
	...
	TerrainSection * section = sections_map[section_z_coord * map_width + section_x_coord];
...
LandscapePiece* Landscape::getTerrainArea(Vector3f const& center, float radius)
{
	...
	for (int i = world_vertex_space.BottomBorder; i <= world_vertex_space.TopBorder; ++i) {
		for (int j = world_vertex_space.LeftBorder; j <= world_vertex_space.RightBorder; ++j) {
			...
			TerrainSection * section = sections_map[section_position.y * map_width + section_position.x];
			if (section == nullptr) {
				if (j % section_size == 0) {
