...
namespace te {
	...
	void BasicTool::ModifyTerrain(Terrain* terrain, Vector3f interPoint)
	{
		if (terrain != nullptr && m_brush != nullptr)
		{
			m_interPoint = interPoint;
			TerrainArea* area = terrain->GetArea({ interPoint.x, interPoint.z }, m_brush->size);
			ModifyPositions(area);
			ModifyNormals(area);
	...
	void BasicTool::ModifyNormals(TerrainArea* area)
	{
		...
		for (size_t z = 1; z < area->Height - 1; ++z)
		{
			for (size_t x = 1; x < area->Width - 1; ++x) 
			{
				...
				/*Vector3f right = vertex_ref[z * area->Width + x + 1];
				Vector3f left = vertex_ref[z * area->Width + x - 1];
				Vector3f top = vertex_ref[(z + 1) * area->Width + x];
				Vector3f bottom = vertex_ref[(z - 1) * area->Width + x];
				Vector3f topLeft = vertex_ref[(z + 1) * area->Width + x - 1];
				Vector3f bottomRight = vertex_ref[(z - 1) * area->Width + x + 1];*/
	...
	SculptTool::SculptTool(float strength)
	:	BasicTool(), 
		m_strength(strength) 
	{}

	void SculptTool::ModifyPositions(TerrainArea* area)
	{
		static std::chrono::steady_clock::time_point time = std::chrono::steady_clock::now();
		static auto lastInterPoint = m_interPoint;
		static float strength_mod = 0;
		if (lastInterPoint == m_interPoint) {
			...
			lastInterPoint = m_interPoint;
		}

		strength_mod = std::clamp<float>(strength_mod, MIN_STRENGTH_MOD, 1.f);

		Vector2f xz_intersection_point{ m_interPoint.x, m_interPoint.z };
		...
		for (auto dataIndex : area->Map) {
			...
			{
				float increment = m_strength * m_brush->GetFalloffFactor(distance) * strength_mod;
				vertex_position.y += increment;
			}
		}
	}
	
	SmoothTool::SmoothTool(float strength, uint8_t levels)
	:	BasicTool(), 
		m_strength(strength), 
		m_levels(levels) 
	{}
	
	void SmoothTool::ModifyPositions(TerrainArea* area)
	{
		std::vector<Vector3f> positions;
	
		Vector2f xzIntersectionPoint{ m_interPoint.x, m_interPoint.z };
