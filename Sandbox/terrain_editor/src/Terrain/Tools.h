...
namespace te
{
    ...
    public:
        ...
        virtual void Merge(BasicToolCommand *command)
        {
            ...
            AABB2Di mcAABB{
                mcFi,
                Vector2i{ int32_t(mcTerrainArea->Width), int32_t(mcTerrainArea->Height) } + mcFi
            };

            bool addCancelPair = true;

            static std::initializer_list<AABB2Di> nullAABB{ AABB2Di{ Vector2i{ 0 },
                                                                     Vector2i{ 0 } } };
            ...
            for (size_t i = 0; i < m_areasToCancel.size(); i++)
            {
                ...
                else
                {
                    if (addCancelPair)
                    {
                        AABB2Di cancelAreaAABB{ fi,
                                                Vector2i{ int32_t(cancelPair.first->Width),
                                                          int32_t(cancelPair.first->Height) } +
                                                    fi };
            ...
            if (!addCancelPair)
                m_areasToCancel.push_back({ mcTerrainArea, nullAABB });
            else
                m_areasToCancel.push_back(mcCancelPair);
    ...
    {
    public:
        SmoothToolCommand(Terrain *terrain,
                          Vector3f interPoint,
                          Brush brush,
                          float strength,
                          uint32_t levels)
        ...
        void Execute() override
        {
            ...
            {
                for (auto area : m_areasToExecute)
                    m_terrain->ApplyAreaParts(area.first, area.second);
            }
        }
    ...
    public:
        ToolManager(Terrain *terrain) : m_terrain(terrain), m_merge(false) {}
        ...
        void ApplySculpt(Brush brush, Vector3f interPoint, float strength)
        {
            ...
            {
                strength_mod = (std::chrono::duration_cast<std::chrono::milliseconds>(
