...
namespace te {
	...
	{
		return Vector2i{
			int32_t((floatCoord.x + offset.x - CORRECTIVE_VALUE) / den),
	...
	Terrain::Terrain(TerrainInitSize columns, TerrainInitSize rows, Rainy::Image* heightMap, Rainy::Image* normalsMap, float heightMod)
	{
		...
		if (heightMap != nullptr &&
			heightMap->GetChannels() == 1 &&
			heightMap->GetWidth() <= TERRAIN_RES &&
			heightMap->GetWidth() <= TERRAIN_RES)
		{
		...
		for (float y = 0; y < rowsCount; y++)
		{
			for (float x = 0; x < columnsCount; x++)
			{
				...
				section->Position = Vector2f{ x * WORLD_SECTION_SIZE, y * WORLD_SECTION_SIZE } + Vector2f{ posOffsetX, posOffsetY };
				section->TexturePosition = Vector2f{ x * TEX_SECTION_SIZE, y * TEX_SECTION_SIZE } + Vector2f{ texOffsetU, texOffsetV };

				m_sectionsMap[(y + sectionMapOffsetY) * SECTION_TERRAIN_SIZE + x + sectionMapOffsetX] = section;
		...
		m_heightMapTexture->TextureData(TERRAIN_RES, TERRAIN_RES, 1, Rainy::TextureDataType::FLOAT, (void*)m_heights.data());
		...
		m_normalMapTexture->TextureData(TERRAIN_RES, TERRAIN_RES, 3, Rainy::TextureDataType::FLOAT, (void*)m_normals.data());
	...
	void Terrain::AddSection(Vector2f point)
	{
		...
		Vector2i newSectionCoord = ToIntCoord(point, Vector2f{ WORLD_HALF_TERRAIN_SIZE }, WORLD_SECTION_SIZE);
	...
	void Terrain::RemoveSection(Vector2f point)
	{
		...
		delete section;
		Vector2i sectionCoord = ToIntCoord(point, Vector2f{ WORLD_HALF_TERRAIN_SIZE }, WORLD_SECTION_SIZE);
	...
	{
		Rainy::AABB2Df recArea = {
			{ point.x - radius + WORLD_HALF_TERRAIN_SIZE, point.y - radius + WORLD_HALF_TERRAIN_SIZE },
			{ point.x + radius + WORLD_HALF_TERRAIN_SIZE, point.y + radius + WORLD_HALF_TERRAIN_SIZE }
		};
		if (recArea.Min.x < 0) recArea.Min.x = 0;
		if (recArea.Max.x > WORLD_TERRAIN_SIZE) recArea.Max.x = WORLD_TERRAIN_SIZE;
		if (recArea.Min.y < 0) recArea.Min.y = 0;
		if (recArea.Max.y > WORLD_TERRAIN_SIZE) recArea.Max.y = WORLD_TERRAIN_SIZE;

		Vector2i firstCoord = { int(recArea.Min.x / WORLD_PART_SIZE), int(recArea.Min.y / WORLD_PART_SIZE) };
		Vector2i lastCoord = { int(recArea.Max.x / WORLD_PART_SIZE), int(recArea.Max.y / WORLD_PART_SIZE) };
		...
		for (int32_t y = firstCoord.y; y <= lastCoord.y; y++)
		{
			for (int32_t x = firstCoord.x; x <= lastCoord.x; x++)
			{
				...
				{
					Vector3f position = { currPoint2d.x, m_heights[y * TERRAIN_RES + x], currPoint2d.y };
	...
	void Terrain::ApplyArea(TerrainArea* area)
	{
		...
		for (int32_t y = 0; y < area->Height; y++)
		{
			for (int32_t x = 0; x < area->Width; x++)
			{
				...
				if (m_heights[terrIndex] > m_maxHeight) m_maxHeight = m_heights[terrIndex];
				else if (m_heights[terrIndex] < m_minHeight) m_minHeight = m_heights[terrIndex];
	...
	Rainy::Image* Terrain::GetHeightMapImage() const
	{
		...
		if (heightDiff < HEIGHT_MOD) heightDiff = HEIGHT_MOD;
	...
	bool Terrain::GetSurfacePoint(float x, float z, Vector3f& dest) const
	{
		...
		Vector3f v1 = { 0, m_heights[(heightCoord.y + 1) * TERRAIN_RES + heightCoord.x], WORLD_PART_SIZE };
		Vector3f v2 = { WORLD_PART_SIZE, m_heights[heightCoord.y * TERRAIN_RES + (heightCoord.x + 1)], 0 };
		...
		if (localPosition.x > (WORLD_PART_SIZE - (localPosition.y)))
			v0 = { WORLD_PART_SIZE, m_heights[(heightCoord.y + 1) * TERRAIN_RES + (heightCoord.x + 1)], WORLD_PART_SIZE };
	...
	{
		Rainy::AABB2Di borders{ { int32_t(SECTION_TERRAIN_SIZE) - 1, int32_t(SECTION_TERRAIN_SIZE) - 1} , { 0, 0 } };
		for (size_t y = 0; y < SECTION_TERRAIN_SIZE; y++)
		{
			for (size_t x = 0; x < SECTION_TERRAIN_SIZE; x++)
			{
				if (m_sectionsMap[y * SECTION_TERRAIN_SIZE + x] == nullptr) continue;

				if (borders.Min.x > x) borders.Min.x = x;
				else if (borders.Max.x < x) borders.Max.x = x;

				if (borders.Min.y > y) borders.Min.y = y;
				else if (borders.Max.y < y) borders.Max.y = y;
			}
		}
		borders.Max.add( { 1, 1 } );
		return borders;
