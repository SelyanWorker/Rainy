...
namespace te
{
    ...
    //		delete area;
    //	}
    //	//std::cout << "Time: " << ((std::chrono::high_resolution_clock::now()) -
    //start_point).count() / 1000 << std::endl;
    //}
    ...
    //			{
    //				if (leftIndex != TerrainArea::INCORRECT_INDEX) sum +=
    //NormalCalc(area->Positions[leftIndex], area->Positions[topLeftIndex], center); 				if (topIndex !=
    //TerrainArea::INCORRECT_INDEX) sum += NormalCalc(area->Positions[topIndex],
    //area->Positions[topLeftIndex], center) * -1.f;
    //			}

    //			if (bottomRightIndex != TerrainArea::INCORRECT_INDEX)
    //			{
    //				if (bottomIndex != TerrainArea::INCORRECT_INDEX) sum +=
    //NormalCalc(area->Positions[bottomIndex], center, area->Positions[bottomRightIndex]); 				if
    //(rightIndex != TerrainArea::INCORRECT_INDEX) sum += NormalCalc(area->Positions[rightIndex],
    //center, area->Positions[bottomRightIndex]) * -1.f;
    //			}

    //			if (topIndex != TerrainArea::INCORRECT_INDEX && rightIndex !=
    //TerrainArea::INCORRECT_INDEX) 				sum += NormalCalc(center, area->Positions[topIndex],
    //area->Positions[rightIndex]); 			if (bottomIndex != TerrainArea::INCORRECT_INDEX && leftIndex !=
    //TerrainArea::INCORRECT_INDEX) 				sum += NormalCalc(center, area->Positions[leftIndex],
    //area->Positions[bottomIndex]) * -1.f;
    ...
    void ScupltToolCommand::ModifyPositions(TerrainArea *area, Vector3f interPoint)
    {
        ...
        {
            if (dataIndex == area->INCORRECT_INDEX)
                continue;
            ...
            for (int x = 0; x < area->Width; x++)
            {
                ...
                for (; zBegin <= zEnd; zBegin++)
                {
                    for (; xBegin <= xEnd; xBegin++)
                    {
                        ...
                        if (index != TerrainArea::INCORRECT_INDEX)
                            heightSum += area->Positions[index].y;
                    }
                }
