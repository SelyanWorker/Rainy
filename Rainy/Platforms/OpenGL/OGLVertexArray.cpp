...
namespace Rainy {
	...
	{
		RN_ASSERT(buffers.size() <= MAX_VERTEX_BUFFERS,
			"buffers.size() > m_vertexBuffers.size()");
		Bind();
		GLuint index = 0;
		for(auto initBuffer : buffers)
		{
			if (initBuffer == nullptr) continue;
			...
			for (auto const& element : bufferLayout)
			{
				...
				switch (element.Type)
				{
				case FLOAT2: type = GL_FLOAT; break;
				case FLOAT3: type = GL_FLOAT; break;
				case NONE: RN_CORE_ERROR("SetVertexBuffer element no type");
				default: RN_CORE_ERROR("From SetVertexBuffers -> default in type switch");
				}
				glVertexAttribPointer(index, element.Size, type, normalize, stride, (void*)element.Offset);
	...
	std::vector<VertexBuffer*> OGLVertexArray::GetVertexBuffers() const
	{
		return m_vertexBuffers;
	}

	IndexBuffer* OGLVertexArray::GetIndexBuffer() const
	{
