...
namespace Rainy {
	...
	{
		for (auto texturePtr : colorTextures) m_colorTextures.push_back(static_cast<OGLTexture2D*>(texturePtr));
		...
		for (size_t i = 0; i < m_colorTextures.size(); i++)
		{
			...
			glFramebufferTexture2D(GL_FRAMEBUFFER, drawColorTextures.back(), GL_TEXTURE_2D, m_colorTextures[i]->GetNative(), 0);
		}
	...
	std::pair<float, float> OGLFrameBuffer::GetSize()
	{
